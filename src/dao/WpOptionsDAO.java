...
public class WpOptionsDAO extends BaseHibernateDAO {
	...
	public void update(WpOptions transientInstance) {
	...
	public WpOptions findById(java.lang.Long id) {
		log.debug("getting WpOptions instance with id: " + id);
		try {
			WpOptions instance = (WpOptions) getSession().get("model.WpOptions",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(WpOptions instance) {
		...
		try {
			List results = getSession().createCriteria("model.WpOptions")
